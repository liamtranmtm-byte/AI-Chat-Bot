...
2. **Gửi kèm ảnh** (`src/zaloClient.js` — `sendImageMessage`, `src/driveImages.js`) — khi bot xác định đúng 1 mẫu cụ thể **còn hàng**, nó gửi ảnh thật (loại tin `image`). Ảnh lấy từ **folder Google Drive** (đặt tên file theo mã sản phẩm) và phục vụ lại qua `/img/<ID>`, hoặc từ URL thật đặt tay trong cột "Link ảnh". Áp dụng luôn cho `/demo-chat`. Xem mục "Ảnh sản phẩm thật" bên dưới. *(Chiều khách gửi ảnh nhờ định giá — để dành giai đoạn sau.)*
...
### Ảnh sản phẩm thật (Google Drive, đặt tên theo mã)

Để bot gửi **ảnh thật** thay vì ảnh test, quản lý ảnh trong 1 folder Google Drive — không cần dán link vào Sheet:

1. Tạo 1 folder Drive (vd "Ảnh STWatch").
2. **Đặt tên mỗi file ảnh theo đúng mã sản phẩm**: `RL001.jpg`, `OM001.png`, `SK001.jpg`... (tên có thêm chữ phía sau vẫn nhận, vd `RL001 - mặt trước.jpg` → khớp `RL001`).
3. **Share folder** cho email service account (hoặc "Anyone with link: Viewer").
4. Set env `DRIVE_IMAGE_FOLDER_ID` = ID folder (lấy từ URL `.../folders/<ID>`).

Xong. Thả thêm file vào folder là bot tự cập nhật (cache ~5 phút), **không cần sửa Sheet, không deploy lại**.

**Cách bot chọn ảnh (thứ tự ưu tiên):**
1. Nếu cột "Link ảnh" trong Sheet có **URL thật** (không phải `placehold.co`) → dùng URL đó (ghi đè thủ công).
2. Ngược lại, nếu folder Drive có file khớp mã → phục vụ qua `https://ten-app.onrender.com/img/<ID>`.
3. Không có ảnh thật → **không gửi ảnh** (không gửi ảnh test).

> Ảnh được phục vụ lại qua chính domain của app (`/img/<ID>`) chứ không hotlink thẳng Drive → Zalo và trình duyệt tải ổn định hơn. Trên Render, biến `RENDER_EXTERNAL_URL` có sẵn nên link ảnh tự thành tuyệt đối; host khác thì set `PUBLIC_BASE_URL`.

Service account cần scope Drive (đã có sẵn trong code) — chỉ cần **bật Google Drive API** trong cùng project ở Google Cloud Console (APIs & Services → Library → Google Drive API → Enable).
